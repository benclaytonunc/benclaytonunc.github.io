...
Spark = function () {
  ...
  this.findCollisionCanidates = function () {
    ...
    var canidates = [];
    if (cn.nextSpark) canidates.push(cn.nextSpark);
    if (cn.north.nextSpark) canidates.push(cn.north.nextSpark);
    if (cn.south.nextSpark) canidates.push(cn.south.nextSpark);
    if (cn.east.nextSpark) canidates.push(cn.east.nextSpark);
    if (cn.west.nextSpark) canidates.push(cn.west.nextSpark);
    if (cn.north.east.nextSpark) canidates.push(cn.north.east.nextSpark);
    if (cn.north.west.nextSpark) canidates.push(cn.north.west.nextSpark);
    if (cn.south.east.nextSpark) canidates.push(cn.south.east.nextSpark);
    if (cn.south.west.nextSpark) canidates.push(cn.south.west.nextSpark);
    return canidates
  };
...
Ship = function () {
  ...
  this.children.exhaust = new Spark();
...
BigAlien = function () {
  ...
  this.children.bottom = new Spark();
  ...
  this.preMove = function (delta) {
    ...
    var topCount = 0;
    if (cn.north.nextSpark) topCount++;
    if (cn.north.east.nextSpark) topCount++;
    if (cn.north.west.nextSpark) topCount++;

    var bottomCount = 0;
    if (cn.south.nextSpark) bottomCount++;
    if (cn.south.east.nextSpark) bottomCount++;
    if (cn.south.west.nextSpark) bottomCount++;
    ...
    if (this.bulletCounter <= 0) {
      ...
      for (var i = 0; i < this.bullets.length; i++) {
        ...
        //  bullet.vel.x = 6 * vectorx;
          bullet.vel.x = 10 * vectorx;
        //  bullet.vel.y = 6 * vectory;
        bullet.vel.y = 10 * vectory;
          bullet.visible = true;
          SFX.laser();
          break;
        }
...
Asteroid = function () {
  ...
  this.collision = function (other) {
    ...
    if (this.scale > 0.5) {
      ...
      for (var i = 0; i < 3; i++) {
        var frag = $.extend(true, {}, this);
        frag.vel.x = Math.random() * 6 - 3;
        frag.vel.y = Math.random() * 6 - 3;
        if (Math.random() > 0.5) {
          frag.points.reverse();
        }
        frag.vel.rot = Math.random() * 2 - 1;
        frag.move(frag.scale * 3); 
        Game.sprites.push(frag);
      }
    }
    Game.explosionAt(other.x, other.y);
    this.die();
...
GridNode = function () {
  ...
  this.enter = function (sprite) {
    sprite.nextSpark = this.nextSpark;
    this.nextSpark = sprite;
  };

  this.leave = function (sprite) {
    var ref = this;
    while (ref && (ref.nextSpark != sprite)) {
      ref = ref.nextSpark;
    }
    if (ref) {
      ref.nextSpark = sprite.nextSpark;
      sprite.nextSpark = null;
    }
  };

  this.eachSpark = function(sprite, callback) {
    var ref = this;
    while (ref.nextSpark) {
      ref = ref.nextSpark;
...
SFX = {
  laser:     new Audio('heat-vision.mp3'),
  explosion: new Audio('SFB-explosion2.mp3')
};
...
Game = {
  ...
  spawnAsteroids: function (count) {
    ...
    for (var i = 0; i < count; i++) {
      var frag = new Asteroid();
      frag.x = Math.random() * this.canvasWidth;
      frag.y = Math.random() * this.canvasHeight;
      while (!frag.isClear()) {
        frag.x = Math.random() * this.canvasWidth;
        frag.y = Math.random() * this.canvasHeight;
      }
      frag.vel.x = Math.random() * 4 - 2;
      frag.vel.y = Math.random() * 4 - 2;
      if (Math.random() > 0.5) {
        frag.points.reverse();
      }
      frag.vel.rot = Math.random() * 2 - 1;
...
$(function () {
  ...
  Spark.prototype.context = context;
  Spark.prototype.grid    = grid;
  Spark.prototype.matrix  = new Matrix(2, 3);
  ...
  var mainLoop = function () {
    ...
    Text.renderText(score_text, 24, Game.canvasWidth - 22 * score_text.length, 26);
